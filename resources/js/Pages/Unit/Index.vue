                    ...
                    <Container  >
                        ...
                        buttons-pagination>
                            <template #item-date="{ date }">
                                <div v-if="date">{{moment(date).format('DD-MM-yy')}}</div>
                                <div v-else>---</div>
                            </template>
                            ...
                            <template #item-aksi="{ id }">
                                <Link @click="loading()" :href="route('unit.procurement.show', id)" class="btn btn-xs text-first bg-slate-200 border-none hover:text-white hover:bg-first font-bold" >Lihat</Link>
                            </template>
                        </EasyDataTable>
                    </Container>
...
const chartOptions = {
    ...
    onClick: (event, array) => {
        console.log(array,props.groupedProc)
        data.preview    = props.groupedProc[array[0].index + 1]
        data.show       = 1
    }
