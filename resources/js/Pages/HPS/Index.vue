                    ...
                    <Container  >
                        ...
                        buttons-pagination>
                            ...
                            <template #item-aksi="{ id }">
                                <Link @click="loading()" :href="route('hps.procurement.show', id)" class="btn btn-xs text-first bg-slate-200 border-none hover:text-white hover:bg-first font-bold" >Lihat</Link>
                            </template>
                        </EasyDataTable>
                    </Container>
...
const label = ['Pembuatan HPS', 'Pengajuan HPS', 'HPS ditolak', 'HPS selesai']
...
const chartOptions = {
    ...
    onClick: (event, array) => {
        ...
        if(array[0].index + 5 >= 8){
            ...
            data.preview = props.groupedProc[array[0].index+5]
        }
        data.show       = 1
    }
...
const headers = [
    ...
    { text: "Nilai RAB",        value: "rab_total"  , sortable: true },
