                            ...
                            <div class="collapse-content flex mt-5 flex-wrap">
                                <Container v-for="user in data" :key="user.id" class="w-3/12 mr-5 mb-5">
                                    ...
                                    <div class="flex justify-between ">
                                        ...
                                        <Link @click="loading()" v-if="user.status == 1" :href="route('admin.user.edit', user.id)" class="btn btn-xs text-first bg-slate-100 border-none hover:text-white hover:bg-first font-bold px-4">Edit</Link>
                                    </div>
                                </Container>
                            </div>
                        </div>
...
const stringRole = ['Admin','Pejabat Pembuat Komitmen','Pejabat Pengadaan', 'Tim HPS', 'Tim Kontrak', 'Tim Teknis', 'Unit', 'RAB Checker'];
// const role = () => {

const nonActive = (id, name, loading) =>{
    const x = useForm()
