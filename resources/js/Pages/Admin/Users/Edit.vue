<template>
    <Master>
        <template v-slot="{ loading }" }>
            <Header1 title="Ubah Data Akun" widthSize="100" class="mb-5" />
    ...
    <!-- 
                ...
                <div class="flex justify-end mt-10">
                    ...
                    <button @click="Ubah(user.id, loading)" class="btn border-first modal-button bg-first ml-3">Ubah</button>
                </div>
            </Container>
        </template>
    </Master>
...
const stringRole = ['Admin','Pejabat Pembuat Komitmen (PPK)','Pejabat Pengadaan (PP)', 'Tim HPS', 'Tim Kontrak', 'Tim Teknis', 'Unit', 'RAB Checker'];
...
const Ubah = (id, loading) =>{
    ...
    data.post(route("actor.update", id),{
        ...
        onStart:    (e) => {data.clearErrors(); loading()},
        onFinish:   (e) => {
            loading()
