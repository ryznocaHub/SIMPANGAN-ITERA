                    ...
                    <Container  >
                        ...
                        buttons-pagination>
                            ...
                            <template #item-aksi="{ id }">
                                <Link @click="loading()" :href="route('siren.procurement.show', id)" class="btn btn-xs text-first bg-slate-200 border-none hover:text-white hover:bg-first font-bold" >Lihat</Link>
                            </template>
                        </EasyDataTable>
                    </Container>
...
function filterLength (datas) {
    ...
    const lastLoop  = 3
    const firstLoop = 2
    //looping count by minimum status (3)
...
const label = ['Pengecekan Akun', 'RAB ditolak', 'RAB diterima']
...
const chartOptions = {
    ...
    onClick: (event, array) => {
        data.preview = []
        if(array[0].index + 2 > 3){
            for (let i = 4; i < 11; i++) {
                if(props.groupedProc[i]) {
                    console.log(i,props.groupedProc[i])
