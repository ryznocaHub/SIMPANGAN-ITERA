<template>
    <Master>
        <Container v-if="suppliers.length != 0" >
            ...
            <Link v-if="user.role > 4" :href="route('kontrak.supplier.create')" class="btn btn-sm mt-5 mb-5 text-first bg-slate-200 border-none hover:text-white hover:bg-first  font-bold" >Tambah Data Supplier</Link>
            <EasyDataTable :headers="headers" :items="suppliers" buttons-pagination>
                ...
                <template #item-aksi="{ entity,id }">
                    <Link v-if="entity && user.role==1" :href="route('admin.supplier.show', id)" class="btn btn-xs btn-outline text-first font-bold" >Lihat</Link>
                    <Link v-else-if="entity" :href="route('kontrak.supplier.show', id)" class="btn btn-xs btn-outline text-first font-bold" >Lihat</Link>
